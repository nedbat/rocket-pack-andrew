...
class Alien(Sprite.SimpleSprite):
    ...
    def newDirection(self):
        ...
        if self.target and random.randrange(1, alienShotChance) == 1:
...
class Boss(Sprite.SimpleSprite):
    ...
    def collideBullet(self, bullet):
        ...
        if self.health == 0:
            self.sa.removeSprite(self)
            BossExplosion(self.sa, *self.rect.center)
        else:
...
class BossExplosion(Explosion):
    def __init__(self, sa, x, y):
        Explosion.__init__(self, sa, x, y, 'xplosionbig.bmp', 30)
...
class Gauge(Sprite.Sprite):
    ...
    def update(self):
        ...
        self.surface.fill(self.color, (0, 0, self.rect.width*value/self.maxValue, self.rect.height))
...
class RocketPackAndrew:
    ...
    def initLevel(self, levelNum):
        ...
        newguy = Guy(self.sa)
        if self.guy and not self.guy.died:
        ...
        if (levelNum % len(levels)) == len(levels)-1:
